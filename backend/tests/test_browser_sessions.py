...
def sign_in(client):
    credentials = {
        "email": f"browser-{uuid4().hex}@neurox.test",
        "password": "BrowserTest!123",
    }
    assert (
        client.post(
            "/auth/register", json={**credentials, "name": "Browser User"}
        ).status_code
        == 201
    )
...
def test_browser_cookie_rotation_and_logout(client):
    ...
    assert (
        ...
        and "Path=/auth/browser" in cookie
    )
    ...
    assert (
        client.post("/auth/refresh", json={"refresh_token": old_token}).status_code
        == 401
    )
    ...
    assert (
        client.post("/auth/refresh", json={"refresh_token": current_token}).status_code
        == 401
    )
