...
def stranger_headers(client):
    ...
    response = client.post("/auth/register", json={
        "name": "Unassigned Caregiver", "email": email, "password": "MatrixTest!123",
    })
...
def owned_records(client, patient_headers):
    contact = client.post(
        ...
        json={"name": "Matrix Contact", "phone": "+91 90000 12345", "relationship": "Family", "priority": 4},
    )
    ...
    reminder = client.post(
        ...
        headers=patient_headers,
        json={
            "patient_id": "maya-demo", "type": "hydration", "title": "Matrix Reminder",
...
def test_unassigned_caregiver_cannot_read_patient_domains(client, stranger_headers, path):
...
def test_unassigned_caregiver_cannot_create_or_change_contacts(
    ...
    before = client.get("/patients/maya-demo/emergency-contacts", headers=patient_headers).json()
    create = client.post(
        "/patients/maya-demo/emergency-contacts", headers=stranger_headers,
        json={"name": "Intruder Contact", "phone": "+91 90000 99999", "relationship": "Unknown"},
    )
    update = client.put(
        f"/patients/maya-demo/emergency-contacts/{owned_records['contact']}",
        headers=stranger_headers, json={"active": False},
    )
    assert create.status_code == update.status_code == 403
    assert client.get("/patients/maya-demo/emergency-contacts", headers=patient_headers).json() == before
...
def test_unassigned_caregiver_cannot_create_change_or_delete_reminders(
    ...
    create = client.post("/reminders", headers=stranger_headers, json={
        "patient_id": "maya-demo", "type": "appointment", "title": "Unauthorized reminder",
    ...
    delete = client.delete(f"/reminders/{owned_records['reminder']}", headers=stranger_headers)
    assert create.status_code == update.status_code == delete.status_code == 403
    assert client.get("/patients/maya-demo/reminders", headers=patient_headers).json() == before


@pytest.mark.parametrize(("method", "path", "payload"), [
    ("put", "/patients/maya-demo/safety/settings", {"safe_zone_radius_m": 999}),
    ("post", "/patients/maya-demo/location-updates", {
        "latitude": 26.1, "longitude": 91.7, "accuracy_m": 10,
        "captured_at": datetime.now(timezone.utc).isoformat(),
    }),
    ("post", "/patients/maya-demo/sos-events", {"message": "Unauthorized event"}),
    ("post", "/patients/maya-demo/safety-alerts/not-found/acknowledge", {"note": "Unauthorized"}),
    ("post", "/patients/maya-demo/sos-events/not-found/acknowledge", {"note": "Unauthorized"}),
])
def test_unassigned_caregiver_cannot_mutate_safety_domain(
    client, stranger_headers, method, path, payload
):
    assert client.request(method, path, headers=stranger_headers, json=payload).status_code == 403
...
def test_caregivers_cannot_submit_patient_activity_sessions(client, stranger_headers, path):
    now = datetime.now(timezone.utc)
    payload = {
        "user_id": "maya-demo", "difficulty_level": 2,
        "started_at": now.isoformat(), "event_id": f"matrix-{uuid4().hex}",
    }
    if path.endswith("/complete"):
        payload.update({
            "activity_id": "memory-match", "completed_at": (now + timedelta(seconds=30)).isoformat(),
            "accuracy": .8, "response_time": 5, "attempts": 1, "completion_status": "completed",
        })
