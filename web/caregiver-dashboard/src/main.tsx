...
function App() {
  ...
  useEffect(() => {
    ...
    // Fetch language config and match Maya's preferred language (Assamese)
    fetch(`${apiUrl}/language-config`)
      .then(r => r.ok ? r.json() as Promise<LanguageConfig[]> : Promise.reject())
      .then(configs => { const assamese = configs.find(c => c.languageCode === 'as-IN'); if (assamese) setPatientLang(assamese) })
      .catch(() => { /* use null; badge simply won't render */ })
  }, [user])
  ...
  return <div className="app-shell">
    <aside><div className="brand"><span className="logo">N</span><span>neuro<span>X</span></span></div><nav>{nav.map(({label,icon:Icon})=><button key={label} className={active===label?'selected':''} onClick={()=>setActive(label)}><Icon size={20}/>{label}{label==='Alerts'&&<i>2</i>}</button>)}</nav><div className="sidebar-bottom"><button><Settings size={20}/>Settings</button><button className="user" onClick={signOut} title="Sign out"><div className="avatar">{initials(user.name)}</div><div><b>{user.name}</b><small>Caregiver · Sign out</small></div><MoreHorizontal size={20}/></button></div></aside>
...
function SignIn({onAuthenticated}:{onAuthenticated:(user:AuthUser)=>void}) {
  ...
  return <main className="auth-page"><section className="auth-card"><div className="brand"><span className="logo">N</span><span>neuro<span>X</span></span></div><p className="eyebrow">CAREGIVER PORTAL</p><h1>Welcome back</h1><p className="subtitle">Sign in to view supportive activity and safety information.</p><form onSubmit={submit}><label>Email<input value={email} onChange={e=>setEmail(e.target.value)} type="email" required/></label><label>Password<input value={password} onChange={e=>setPassword(e.target.value)} type="password" minLength={8} required/></label>{error&&<p className="auth-error">{error}</p>}<button className="auth-submit" disabled={loading}>{loading?'Signing in…':'Sign in'}</button></form><div className="or"><span/>or continue with<span/></div><div id="google-button" className="google-placeholder">Google sign-in appears after you set VITE_GOOGLE_CLIENT_ID.</div><p className="auth-foot">Demo: anita@neurox.demo / NeuroXDemo!2026</p></section></main>
}
function Stat({icon,label,value,detail,tone}:{icon:React.ReactNode,label:string,value:string,detail:string,tone:string}) {return <article className="stat"><div className={`stat-icon ${tone}`}>{icon}</div><p>{label}</p><h2>{value}</h2><small>{detail}</small></article>}
function Alert({color,title,detail,label}:{color:string,title:string,detail:string,label:string}) {return <div className="alert-row"><span className={`dot ${color}`}/><div><b>{title}</b><p>{detail}</p></div><span className={`tag ${color}`}>{label}</span><ChevronRight size={18}/></div>}
export default App
